...
simulated instead, which keeps warden's zero-dependency guarantee testable in CI and
...
from warden import Monitor, RunRecorder, Session
...
def test_works_without_langchain_installed():
    ...
    handler = warden_callback(RunRecorder("a"))
...
def test_accepts_a_session_as_well_as_a_recorder():
    ...
    handler = warden_callback(session)
...
def test_recorded_runs_feed_the_monitor():
    ...
    history = []
    for _ in range(200):
        h = warden_callback(RunRecorder("researcher", version="v2", principal="acme"))
    ...
    normal = warden_callback(RunRecorder("researcher", version="v2", principal="acme"))
    ...
    huge = warden_callback(RunRecorder("researcher", version="v2", principal="acme"))
